...
df12_cluster = df12_final %>%
  ...
  summarise(
    ...
    YS_caregiver_depression_24m        = Mode(YS_caregiver_depression_24m, na.rm = TRUE),
...
df12_final = df12_final %>% 
  mutate(
    across(c(YP_stunting_24m, YS_caregiver_depression_24m, X_sex_0m, X_bornearly_0m, X_mother_age_cat_0m, 
...
df22_final = df22 %>%
  ...
  rename(
    ...
    X_GDM_ACDT     = GDM_ACDT,   # history of GDM
    X_MACRO_ACDT   = MACRO_ACDT, # history of macrosomia
...
## Load Data
```{r}
df23 = read_xlsx("raw_data/23/23.xlsx", na = )
```
...
df24_final = df24 %>% 
  ...
  mutate(
    ...
    YS_time_to_catheter = Heure.du.sondage,
