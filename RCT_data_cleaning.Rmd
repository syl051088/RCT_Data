...
df32_final = df32 %>%
  # Rename treatment variable and factor with Control as reference
  rename(Treatment = arm) %>%
  mutate(Treatment = factor(Treatment, levels = c("Control", "Streptomycin"))) %>%
  
  # Create primary outcome variables (YP_ prefix)
  # The primary outcome is radiological assessment at 6 months
  mutate(
    YP_radiologic_6m = factor(radiologic_6m),
    YP_considerable_improvement = factor(ifelse(radiologic_6m == "6_Considerable_improvement", 1, 0)),
    YP_any_improvement = factor(improved)
  ) %>%
  
  # Create secondary outcome variables (YS_ prefix)
  # Based on paper: bacteriological outcomes, clinical condition changes
  mutate(
    YS_strep_resistance = factor(strep_resistance),
    YS_death = factor(ifelse(radiologic_6m == "1_Death", 1, 0))
  ) %>%
  
  # Create covariate variables (X_ prefix with _0w suffix)
  mutate(
    X_gender_0w = factor(gender),
    X_condition_0w = factor(baseline_condition),
    X_temp_0w = factor(baseline_temp),
    X_esr_0w = factor(baseline_esr),
    X_cavitation_0w = factor(baseline_cavitation)
  ) %>%
