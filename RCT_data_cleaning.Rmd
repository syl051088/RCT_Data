...
## Load Data
```{r}
df48 = read_csv("raw_data/48/48.csv")
```
...
# 13. Aggregate to cluster level: numeric → mean, factor → mode
df48_cluster = df48_final %>%
  group_by(ClusterID) %>%
  summarise(
    across(where(is.numeric), ~ mean(.x, na.rm = TRUE)),
    across(where(is.factor), ~ Mode(.x, na.rm = TRUE)[1]),
    .groups = "drop"
  ) %>% 
  select(ClusterID, Treatment, 
               starts_with("YP"),
...
```{r}
# trial 48
write_csv(df48_cluster, "cleaned_data/trial48.csv")
write_rds(df48_cluster, "cleaned_data/trial48.rds")

write_csv(df48_final, "cleaned_data/Clustered_RCT/cluster48.csv")
write_rds(df48_final, "cleaned_data/Clustered_RCT/cluster48.rds")
```
...
## Load Data
```{r}
df50 = read_xlsx("raw_data/50/50.xlsx", na=c("."))
```
...
```{r}
# trial 50
write_csv(df50_cluster, "cleaned_data/trial50.csv")
write_rds(df50_cluster, "cleaned_data/trial50.rds")

write_csv(df50_final, "cleaned_data/Clustered_RCT/cluster50.csv")
write_rds(df50_final, "cleaned_data/Clustered_RCT/cluster50.rds")
```
...
dfc1_cluster = dfc1_final %>%
  ...
  reframe(
    ...
    YP_stunting_24m        = {result = mean(YP_stunting_24m, na.rm = TRUE); ifelse(is.nan(result), NA, result)},
    ...
    X_sex_0m              = {result = mean(X_sex_0m, na.rm = TRUE) - 1; ifelse(is.nan(result), NA, result)},
    X_bornearly_0m        = {result = mean(X_bornearly_0m, na.rm = TRUE); ifelse(is.nan(result), NA, result)},
    X_mother_age_cat_0m   = DescTools::Mode(X_mother_age_cat_0m, na.rm = TRUE),
    X_father_primary_0m   = {result = mean(X_father_primary_0m, na.rm = TRUE); ifelse(is.nan(result), NA, result)},
    X_mother_primary_0m   = {result = mean(X_mother_primary_0m, na.rm = TRUE); ifelse(is.nan(result), NA, result)}
  ) %>%
  ungroup() %>%
  select(
    ClusterID,
...
dfc1_cluster = dfc1_cluster %>% 
  mutate(
    across(c(X_mother_age_cat_0m), as_factor)
  )
```
## Output Data
```{r}
...
# Convert binary factors to have proper factor structure for dfc2_final
dfc2_final = dfc2_final %>% 
  mutate(
    ...
    # Keep all factors as factors (no conversion to numeric)
  )
  ...
  reframe(
    ...
    across(
      ...
      ~ if (is.numeric(.x)) {
          ...
          # Binary factors: convert 1,2 to 0,1 then take mean (gives proportion)
          result = mean(as.numeric(.x) - 1, na.rm = TRUE)
          ifelse(is.nan(result), NA, result)
        } else if (is.factor(.x)) {
