...
update_df = function(trial_no,
  ...
  results_processed = results_processed %>%
    ...
    mutate(
      ...
      !!ifelse(suffix == "", "N_Covariates", paste0("N_Covariates_", suffix)) := N_Covariates,
      Contrast     = contrast
    )
...
df_new = df_new %>%
  rename(
    ...
    precision_gain_SL2 = `How much precision gain can SL2 provide?`,
    ...
    precision_gain_Binom_VS1 = `How much precision gain can Binom_VS1 provide?`,
    ...
    precision_gain_SL2_VS1 = `How much precision gain can SL2_VS1 provide?`,
    ...
    precision_gain_Binom_VS2 = `How much precision gain can Binom_VS2 provide?`,
    ...
    precision_gain_SL2_VS2 = `How much precision gain can SL2_VS2 provide?`,
...
df_summary = df_new %>%
  ...
  summarise(
    ...
    # ── SL1 precision-gain *outlier removed* ─────────────────────
    mean_prec_gain_SL1 = {
      v  = precision_gain_SL1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL1   = {
    ...
    # SL1 precision-gain (full)
    mean_prec_gain_SL1_full     = mean(precision_gain_SL1,  na.rm = TRUE),
    sd_prec_gain_SL1_full       = sd(precision_gain_SL1,    na.rm = TRUE),
    
    # ── SL2 precision-gain *outlier removed* ─────────────────────
    mean_prec_gain_SL2 = {
      v  = precision_gain_SL2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL2   = {
    ...
    # SL2 precision-gain (full)
    mean_prec_gain_SL2_full     = mean(precision_gain_SL2,  na.rm = TRUE),
    sd_prec_gain_SL2_full       = sd(precision_gain_SL2,    na.rm = TRUE),
    
    # ── TMLE precision-gain *outlier removed* ─────────────────────
    mean_prec_gain_TMLE = {
      v  = precision_gain_TMLE
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_TMLE   = {
    ...
    # TMLE precision-gain (full)
    mean_prec_gain_TMLE_full     = mean(precision_gain_TMLE,  na.rm = TRUE),
    sd_prec_gain_TMLE_full       = sd(precision_gain_TMLE,    na.rm = TRUE),
    
    # ── SuperLearner Components (Base) ─────────────────────
    ...
    # ANCOVA_VS1 precision-gain *outlier removed*
    mean_prec_gain_ANCOVA_VS1 = {
      v  = precision_gain_ANCOVA_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_ANCOVA_VS1   = {
      v  = precision_gain_ANCOVA_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # ANCOVA_VS1 precision-gain (full)
    mean_prec_gain_ANCOVA_VS1_full  = mean(precision_gain_ANCOVA_VS1, na.rm = TRUE),
    sd_prec_gain_ANCOVA_VS1_full    = sd(precision_gain_ANCOVA_VS1,   na.rm = TRUE),
    
    # ANHECOVA_VS1 precision-gain *outlier removed*
    mean_prec_gain_ANHECOVA_VS1 = {
      v  = precision_gain_ANHECOVA_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_ANHECOVA_VS1   = {
      v  = precision_gain_ANHECOVA_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # ANHECOVA_VS1 precision-gain (full)
    mean_prec_gain_ANHECOVA_VS1_full  = mean(precision_gain_ANHECOVA_VS1, na.rm = TRUE),
    sd_prec_gain_ANHECOVA_VS1_full    = sd(precision_gain_ANHECOVA_VS1,   na.rm = TRUE),
    
    # Binom_VS1 precision-gain *outlier removed*
    mean_prec_gain_Binom_VS1 = {
      v  = precision_gain_Binom_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_Binom_VS1   = {
      v  = precision_gain_Binom_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # Binom_VS1 precision-gain (full)
    mean_prec_gain_Binom_VS1_full  = mean(precision_gain_Binom_VS1, na.rm = TRUE),
    sd_prec_gain_Binom_VS1_full    = sd(precision_gain_Binom_VS1,   na.rm = TRUE),
    
    # SL1_VS1 precision-gain *outlier removed*
    mean_prec_gain_SL1_VS1 = {
      v  = precision_gain_SL1_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL1_VS1   = {
      v  = precision_gain_SL1_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # SL1_VS1 precision-gain (full)
    mean_prec_gain_SL1_VS1_full  = mean(precision_gain_SL1_VS1, na.rm = TRUE),
    sd_prec_gain_SL1_VS1_full    = sd(precision_gain_SL1_VS1,   na.rm = TRUE),
    
    # SL2_VS1 precision-gain *outlier removed*
    mean_prec_gain_SL2_VS1 = {
      v  = precision_gain_SL2_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL2_VS1   = {
      v  = precision_gain_SL2_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # SL2_VS1 precision-gain (full)
    mean_prec_gain_SL2_VS1_full  = mean(precision_gain_SL2_VS1, na.rm = TRUE),
    sd_prec_gain_SL2_VS1_full    = sd(precision_gain_SL2_VS1,   na.rm = TRUE),
    
    # TMLE_VS1 precision-gain *outlier removed*
    mean_prec_gain_TMLE_VS1 = {
      v  = precision_gain_TMLE_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_TMLE_VS1   = {
      v  = precision_gain_TMLE_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # TMLE_VS1 precision-gain (full)
    mean_prec_gain_TMLE_VS1_full  = mean(precision_gain_TMLE_VS1, na.rm = TRUE),
    sd_prec_gain_TMLE_VS1_full    = sd(precision_gain_TMLE_VS1,   na.rm = TRUE),
    
    # SuperLearner Components VS1
    mean_prec_gain_SL_rpart_VS1 = {
      v  = precision_gain_SL_rpart_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL_rpart_VS1   = {
      v  = precision_gain_SL_rpart_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    mean_prec_gain_SL_rpart_VS1_full  = mean(precision_gain_SL_rpart_VS1, na.rm = TRUE),
    sd_prec_gain_SL_rpart_VS1_full    = sd(precision_gain_SL_rpart_VS1,   na.rm = TRUE),
    
    mean_prec_gain_SL_randomForest_VS1 = {
      v  = precision_gain_SL_randomForest_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL_randomForest_VS1   = {
      v  = precision_gain_SL_randomForest_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    mean_prec_gain_SL_randomForest_VS1_full  = mean(precision_gain_SL_randomForest_VS1, na.rm = TRUE),
    sd_prec_gain_SL_randomForest_VS1_full    = sd(precision_gain_SL_randomForest_VS1,   na.rm = TRUE),
    
    mean_prec_gain_SL_glmnet_VS1 = {
      v  = precision_gain_SL_glmnet_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL_glmnet_VS1   = {
      v  = precision_gain_SL_glmnet_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    mean_prec_gain_SL_glmnet_VS1_full  = mean(precision_gain_SL_glmnet_VS1, na.rm = TRUE),
    sd_prec_gain_SL_glmnet_VS1_full    = sd(precision_gain_SL_glmnet_VS1,   na.rm = TRUE),
    
    mean_prec_gain_SL_gam_VS1 = {
      v  = precision_gain_SL_gam_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL_gam_VS1   = {
      v  = precision_gain_SL_gam_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    mean_prec_gain_SL_gam_VS1_full  = mean(precision_gain_SL_gam_VS1, na.rm = TRUE),
    sd_prec_gain_SL_gam_VS1_full    = sd(precision_gain_SL_gam_VS1,   na.rm = TRUE),
    
    mean_prec_gain_SL_bartMachine_VS1 = {
      v  = precision_gain_SL_bartMachine_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL_bartMachine_VS1   = {
      v  = precision_gain_SL_bartMachine_VS1
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    mean_prec_gain_SL_bartMachine_VS1_full  = mean(precision_gain_SL_bartMachine_VS1, na.rm = TRUE),
    sd_prec_gain_SL_bartMachine_VS1_full    = sd(precision_gain_SL_bartMachine_VS1,   na.rm = TRUE),
    
    # ── VS2 Methods ─────────────────────────────────────────────────────────
    
    # ANCOVA_VS2 precision-gain *outlier removed*
    mean_prec_gain_ANCOVA_VS2 = {
      v  = precision_gain_ANCOVA_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_ANCOVA_VS2   = {
      v  = precision_gain_ANCOVA_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # ANCOVA_VS2 precision-gain (full)
    mean_prec_gain_ANCOVA_VS2_full  = mean(precision_gain_ANCOVA_VS2, na.rm = TRUE),
    sd_prec_gain_ANCOVA_VS2_full    = sd(precision_gain_ANCOVA_VS2,   na.rm = TRUE),
    
    # ANHECOVA_VS2 precision-gain *outlier removed*
    mean_prec_gain_ANHECOVA_VS2 = {
      v  = precision_gain_ANHECOVA_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_ANHECOVA_VS2   = {
      v  = precision_gain_ANHECOVA_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # ANHECOVA_VS2 precision-gain (full)
    mean_prec_gain_ANHECOVA_VS2_full  = mean(precision_gain_ANHECOVA_VS2, na.rm = TRUE),
    sd_prec_gain_ANHECOVA_VS2_full    = sd(precision_gain_ANHECOVA_VS2,   na.rm = TRUE),
    
    # Binom_VS2 precision-gain *outlier removed*
    mean_prec_gain_Binom_VS2 = {
      v  = precision_gain_Binom_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_Binom_VS2   = {
      v  = precision_gain_Binom_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # Binom_VS2 precision-gain (full)
    mean_prec_gain_Binom_VS2_full  = mean(precision_gain_Binom_VS2, na.rm = TRUE),
    sd_prec_gain_Binom_VS2_full    = sd(precision_gain_Binom_VS2,   na.rm = TRUE),
    
    # SL1_VS2 precision-gain *outlier removed*
    mean_prec_gain_SL1_VS2 = {
      v  = precision_gain_SL1_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL1_VS2   = {
      v  = precision_gain_SL1_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      sd(v[v <= ub], na.rm = TRUE)
    },
    # SL1_VS2 precision-gain (full)
    mean_prec_gain_SL1_VS2_full  = mean(precision_gain_SL1_VS2, na.rm = TRUE),
    sd_prec_gain_SL1_VS2_full    = sd(precision_gain_SL1_VS2,   na.rm = TRUE),
    
    # SL2_VS2 precision-gain *outlier removed*
    mean_prec_gain_SL2_VS2 = {
      v  = precision_gain_SL2_VS2
      ub = quantile(v, 0.75, na.rm = TRUE) + 1.5 * IQR(v, na.rm = TRUE)
      mean(v[v <= ub], na.rm = TRUE)
    },
    sd_prec_gain_SL2_VS2   = {
      v  = precision_gain_SL2_VS2
      ub = quantile(v, 0.75, na.
```
...
df_long_full = df_new %>%       # df_new in your workspace
  ...
  mutate(method = dplyr::case_match(
    ...
    "precision_gain_SL2"                     ~ "SL2",
    "precision_gain_TMLE"                    ~ "TMLE",
    
    # VS1 methods
    "precision_gain_ANCOVA_VS1"              ~ "ANCOVA_VS1",
    "precision_gain_ANHECOVA_VS1"            ~ "ANHECOVA_VS1",
    "precision_gain_Binom_VS1"               ~ "Binom_VS1",
    "precision_gain_SL1_VS1"                 ~ "SL1_VS1",
    "precision_gain_SL_rpart_VS1"            ~ "SL_rpart_VS1",
    "precision_gain_SL_randomForest_VS1"     ~ "SL_randomForest_VS1",
    "precision_gain_SL_glmnet_VS1"           ~ "SL_glmnet_VS1",
    "precision_gain_SL_gam_VS1"              ~ "SL_gam_VS1",
    "precision_gain_SL_bartMachine_VS1"      ~ "SL_bartMachine_VS1",
    "precision_gain_SL2_VS1"                 ~ "SL2_VS1",
    "precision_gain_TMLE_VS1"                ~ "TMLE_VS1",
    
    # VS2 methods
    "precision_gain_ANCOVA_VS2"              ~ "ANCOVA_VS2",
    "precision_gain_ANHECOVA_VS2"            ~ "ANHECOVA_VS2",
    "precision_gain_Binom_VS2"               ~ "Binom_VS2",
    "precision_gain_SL1_VS2"                 ~ "SL1_VS2",
    "precision_gain_SL_rpart_VS2"            ~ "SL_rpart_VS2",
    "precision_gain_SL_randomForest_VS2"     ~ "SL_randomForest_VS2",
    "precision_gain_SL_gam_VS2"              ~ "SL_gam_VS2",
    "precision_gain_SL_bartMachine_VS2"      ~ "SL_bartMachine_VS2",
    "precision_gain_SL2_VS2"                 ~ "SL2_VS2"
  ))

###############################################################################
##  1.  Caption builder for ONE outcome type and method type -----------------
###############################################################################
build_caption = function(outcome_type, vs_type = "none") {

  ## Filter methods based on VS type
  method_filter = function(methods) {
    if (vs_type == "none") {
      methods[!grepl("_VS[12]$", methods)]  # No VS1 or VS2
    } else if (vs_type == "VS1") {
      methods[grepl("_VS1$", methods)]      # Only VS1
    } else if (vs_type == "VS2") {
      methods[grepl("_VS2$", methods)]      # Only VS2
    } else {
      methods  # fallback: all methods
    }
  }
  ...
  ## For *continuous* outcome, drop Binom variants from NA line
  if (outcome_type == "continuous") {
    binom_methods = if (vs_type == "none") {
      "Binom"
    } else if (vs_type == "VS1") {
      "Binom_VS1"
    } else if (vs_type == "VS2") {
...
# VS1 methods
df_binary_vs1 = df_long_outlier_removed %>% 
  filter(outcome_group == "binary", grepl("_VS1$", method))
cap_binary_vs1 = build_caption("binary", vs_type = "VS1")

p_binary_vs1 = make_plot(
  df_binary_vs1,
  "Precision Gain vs Unadjusted – Binary (VS1 Methods)",
  cap_binary_vs1
)
ggsave("cleaned_data/Plot/precision_gain_binary_vs1.png", p_binary_vs1, width = 12, height = 6, dpi = 1200)

# VS2 methods
df_binary_vs2 = df_long_outlier_removed %>% 
  filter(outcome_group == "binary", grepl("_VS2$", method))
cap_binary_vs2 = build_caption("binary", vs_type = "VS2")

p_binary_vs2 = make_plot(
  df_binary_vs2,
  "Precision Gain vs Unadjusted – Binary (VS2 Methods)",
  cap_binary_vs2
)
ggsave("cleaned_data/Plot/precision_gain_binary_vs2.png", p_binary_vs2, width = 12, height = 6, dpi = 1200)

###############################################################################
##  4.  Continuous plots (3 plots: Base, VS1, VS2) --------------------------
###############################################################################

# Base methods (no VS)
df_continuous_base = df_long_outlier_removed %>% 
  filter(outcome_group == "continuous", !grepl("_VS[12]$", method))
cap_cont_base = build_caption("continuous", vs_type = "none")
...
# VS1 methods
df_continuous_vs1 = df_long_outlier_removed %>% 
  filter(outcome_group == "continuous", grepl("_VS1$", method))
cap_cont_vs1 = build_caption("continuous", vs_type = "VS1")

p_cont_vs1 = make_plot(
  df_continuous_vs1,
  "Precision Gain vs Unadjusted – Continuous (VS1 Methods)",
  cap_cont_vs1
)
ggsave("cleaned_data/Plot/precision_gain_continuous_vs1.png", p_cont_vs1, width = 12, height = 6, dpi = 1200)

# VS2 methods
df_continuous_vs2 = df_long_outlier_removed %>% 
  filter(outcome_group == "continuous", grepl("_VS2$", method))
cap_cont_vs2 = build_caption("continuous", vs_type = "VS2")

p_cont_vs2 = make_plot(
  df_continuous_vs2,
  "Precision Gain vs Unadjusted – Continuous (VS2 Methods)",
  cap_cont_vs2
)
ggsave("cleaned_data/Plot/precision_gain_continuous_vs2.png", p_cont_vs2, width = 12, height = 6, dpi = 1200)

###############################################################################
##  5.  Display all 6 plots --------------------------------------------------
###############################################################################
cat("Displaying all 6 plots:\n")
cat("Binary outcomes:\n")
p_binary_base
p_binary_vs1  
p_binary_vs2
