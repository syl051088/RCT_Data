...
analyze_rct2 = function(df,
                       ...
                       selection = FALSE,
                       n_select = 3,
                       manual_covariates = NULL,  # NEW ARGUMENT: manually selected covariates (steps 2 & 3)
                       n_cores = parallel::detectCores() - 2,
                       seed = 123) {
  require(dplyr)
  require(RobinCar)
    ...
    if (selection) {
      ...
      if (startsWith(outcome, "YP_")) {
        ...
        if (length(outcome_parts) >= 3) {
          # Extract the middle part(s) and construct baseline variable name
...
cat("\n<------------------Variable Seclection 2------------->\n\n")
res3 = analyze_rct2(df4, selection = T)
```
