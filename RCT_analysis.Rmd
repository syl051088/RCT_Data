...
```{r}
type = rep("continuous", length(unique(analyze_rct(df6)$Outcome)))
demo = c("X_age_0m", "X_sex_0m")
```
...
# SL1, VS2
cat("\n<------------------Variable Seclection 2------------->\n\n")
res3 = analyze_rct(df22, T, 2, demo)
df_comparison = update_df(22, type, res3, "VS2")
```
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df11)$Outcome)))
demo = c("X_sex_0w")
```
...
```{r}
type = rep("binary", length(unique(analyze_rct(df12)$Outcome)))
...
```{r}
n = length(unique(analyze_rct(df14)$Outcome))
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df15)$Outcome)))
demo = c()
```
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df16)$Outcome)))
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df17)$Outcome)))
...
```{r}
n = length(unique(analyze_rct(df18)$Outcome))
...
```{r}
n = length(unique(analyze_rct(df19)$Outcome))
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df20)$Outcome)))
...
```{r}
n = length(unique(analyze_rct(df23)$Outcome))
...
```{r}
n = length(unique(analyze_rct(df26)$Outcome))
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df36)$Outcome)))
demo = c()
```
...
```{r}
type = rep("binary", length(unique(analyze_rct(df37)$Outcome)))
...
```{r}
n = length(unique(analyze_rct(df42)$Outcome))
type = c("time to event", rep("continuous", n-1))
demo = c("X_gender_0h", "X_bmi_0h", "X_age_0h")
```
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df45)$Outcome)))
...
```{r}
type = rep("continuous", length(unique(analyze_rct(df46)$Outcome)))
demo = c("X_Age_0y")
```
