...
df_outlier_removed = df_new %>%
  mutate(
    ...
    precision_gain_SL2 = remove_upper_outliers(precision_gain_SL2),
    ...
    precision_gain_Binom_VS1 = remove_upper_outliers(precision_gain_Binom_VS1),
    ...
    precision_gain_SL2_VS1 = remove_upper_outliers(precision_gain_SL2_VS1),
    ...
    precision_gain_Binom_VS2 = remove_upper_outliers(precision_gain_Binom_VS2),
    ...
    precision_gain_SL2_VS2 = remove_upper_outliers(precision_gain_SL2_VS2)
...
df_long_outlier_removed = df_outlier_removed %>%
  select(outcome_group, sample_size_cat, 
         # Base methods
         precision_gain_ANCOVA, precision_gain_ANHECOVA, precision_gain_Binom, precision_gain_SL1,
         precision_gain_SL_rpart, precision_gain_SL_randomForest, precision_gain_SL_glmnet, 
         precision_gain_SL_gam, precision_gain_SL_bartMachine, precision_gain_SL2, precision_gain_TMLE,
         # VS1 methods
         precision_gain_ANCOVA_VS1, precision_gain_ANHECOVA_VS1, precision_gain_Binom_VS1, precision_gain_SL1_VS1,
         precision_gain_SL_rpart_VS1, precision_gain_SL_randomForest_VS1, precision_gain_SL_glmnet_VS1,
         precision_gain_SL_gam_VS1, precision_gain_SL_bartMachine_VS1, precision_gain_SL2_VS1, precision_gain_TMLE_VS1,
         # VS2 methods
         precision_gain_ANCOVA_VS2, precision_gain_ANHECOVA_VS2, precision_gain_Binom_VS2, precision_gain_SL1_VS2,
         precision_gain_SL_rpart_VS2, precision_gain_SL_randomForest_VS2, precision_gain_SL_gam_VS2,
         precision_gain_SL_bartMachine_VS2, precision_gain_SL2_VS2) %>%
  ...
  mutate(
    method = case_when(
      ...
      method == "precision_gain_Binom_VS1" ~ "Binom_VS1",
      ...
      method == "precision_gain_SL2_VS1" ~ "SL2_VS1",
      ...
      method == "precision_gain_Binom_VS2" ~ "Binom_VS2",
      ...
      method == "precision_gain_SL2_VS2" ~ "SL2_VS2"
    ),
    method = factor(method, levels = c(
      # Base methods
      "ANCOVA", "ANHECOVA", "Binom", "SL1", "SL_rpart", "SL_randomForest", 
      "SL_glmnet", "SL_gam", "SL_bartMachine", "SL2", "TMLE",
      # VS1 methods
      "ANCOVA_VS1", "ANHECOVA_VS1", "Binom_VS1", "SL1_VS1", "SL_rpart_VS1", 
      "SL_randomForest_VS1", "SL_glmnet_VS1", "SL_gam_VS1", "SL_bartMachine_VS1", "SL2_VS1", "TMLE_VS1",
      # VS2 methods
      "ANCOVA_VS2", "ANHECOVA_VS2", "Binom_VS2", "SL1_VS2", "SL_rpart_VS2", 
      "SL_randomForest_VS2", "SL_gam_VS2", "SL_bartMachine_VS2", "SL2_VS2"))
  )

# Color palette for methods (expanded for VS1 and VS2)
method_colors = c(
  # Base methods
  "ANCOVA" = "#1f77b4", "ANHECOVA" = "#ff7f0e", "Binom" = "#2ca02c", "SL1" = "#d62728",
  "SL_rpart" = "#9467bd", "SL_randomForest" = "#8c564b", "SL_glmnet" = "#e377c2", 
  "SL_gam" = "#7f7f7f", "SL_bartMachine" = "#bcbd22", "SL2" = "#17becf", "TMLE" = "#aec7e8",
  # VS1 methods
  "ANCOVA_VS1" = "#1f77b4", "ANHECOVA_VS1" = "#ff9896", "Binom_VS1" = "#98df8a", "SL1_VS1" = "#ffbb78",
  "SL_rpart_VS1" = "#c5b0d5", "SL_randomForest_VS1" = "#c49c94", "SL_glmnet_VS1" = "#f7b6d3", 
  "SL_gam_VS1" = "#c7c7c7", "SL_bartMachine_VS1" = "#dbdb8d", "SL2_VS1" = "#9edae5", "TMLE_VS1" = "#393b79",
  # VS2 methods  
  "ANCOVA_VS2" = "#637939", "ANHECOVA_VS2" = "#8c6d31", "Binom_VS2" = "#843c39", "SL1_VS2" = "#7b4173",
  "SL_rpart_VS2" = "#5254a3", "SL_randomForest_VS2" = "#6b6ecf", "SL_gam_VS2" = "#9c9ede", 
  "SL_bartMachine_VS2" = "#637939", "SL2_VS2" = "#8ca252"
)
...
p_sl2_vs2_individual = create_individual_method_plot("SL2_VS2", "#8ca252")
...
build_caption = function(outcome_type, vs_type = "none") {
  ...
  ## For *continuous* outcome, drop Binom methods from NA line
...
cat("\n--- Continuous: VS1 Methods ---\n")
df_continuous_vs1_simple = create_simple_table("continuous", "VS1")
print(df_continuous_vs1_simple)

cat("\n--- Continuous: VS2 Methods ---\n")
df_continuous_vs2_simple = create_simple_table("continuous", "VS2")
print(df_continuous_vs2_simple)



###############################################################################
##  4. COMBINED COMPARISON TABLE (outlier removed) ---------------------------
###############################################################################

# Create a comparison table showing mean precision gain across all methods and VS types
create_comparison_table = function(outcome_type) {
  
  # Get base methods
  base_data = df_long_outlier_removed %>% 
    filter(outcome_group == outcome_type, !grepl("_VS[12]$", method)) %>%
    group_by(method) %>%
    summarise(Base_Mean = round(mean(precision_gain), 4), Base_N = n(), .groups = "drop") %>%
    mutate(method_base = gsub("_VS[12]$", "", method))
  
  # Get VS1 methods  
  vs1_data = df_long_outlier_removed %>% 
    filter(outcome_group == outcome_type, grepl("_VS1$", method)) %>%
    group_by(method) %>%
    summarise(VS1_Mean = round(mean(precision_gain), 4), VS1_N = n(), .groups = "drop") %>%
    mutate(method_base = gsub("_VS1$", "", method))
  
  # Get VS2 methods
  vs2_data = df_long_outlier_removed %>% 
    filter(outcome_group == outcome_type, grepl("_VS2$", method)) %>%
    group_by(method) %>%
    summarise(VS2_Mean = round(mean(precision_gain), 4), VS2_N = n(), .groups = "drop") %>%
    mutate(method_base = gsub("_VS2$", "", method))
  
  # Merge all together
  comparison = base_data %>%
    full_join(vs1_data, by = "method_base", suffix = c("", "_vs1")) %>%
    full_join(vs2_data, by = "method_base", suffix = c("", "_vs2")) %>%
    select(Method = method_base, 
           Base_Mean, Base_N,
           VS1_Mean, VS1_N, 
           VS2_Mean, VS2_N) %>%
    arrange(Method)
...
cat("\n=== COMPARISON TABLES (OUTLIER REMOVED) ===\n")

cat("\n--- Binary Outcomes: Method Comparison ---\n")
binary_comparison = create_comparison_table("binary")
print(binary_comparison)

cat("\n--- Continuous Outcomes: Method Comparison ---\n") 
continuous_comparison = create_comparison_table("continuous")
