...
```{r}
df31 = read_rds("cleaned_data/trial31.rds")
```
...
df_comparison = update_df(31, c("continuous", "binary"))
```
...
```{r}
df32 = read_rds("cleaned_data/trial32.rds")
```
...
df_comparison = update_df(32, c("binary", "binary", "binary", "binary"))
```
...
```{r}
df33 = read_rds("cleaned_data/trial33.rds")
```
...
```{r}
df34 = read_rds("cleaned_data/trial34.rds")
```
...
```{r}
df35 = read_rds("cleaned_data/trial35.rds")
```
...
```{r}
df36 = read_rds("cleaned_data/trial36.rds")
```
...
```{r}
df37 = read_rds("cleaned_data/trial37.rds")
```
...
```{r}
df38 = read_rds("cleaned_data/trial38.rds") %>% 
...
```{r}
df39 = read_rds("cleaned_data/trial39.rds") %>% 
  select(-YP_event_flag)
```
...
```{r}
df40 = read_rds("cleaned_data/trial40.rds") %>% 
...
```{r}
df41 = read_rds("cleaned_data/trial41.rds")
```
...
```{r}
df42 = read_rds("cleaned_data/trial42.rds") %>% 
...
```{r}
df43 = read_rds("cleaned_data/trial43.rds") %>% 
...
```{r}
df44 = read_rds("cleaned_data/trial44.rds")
```
...
```{r}
df45 = read_rds("cleaned_data/trial45.rds") %>% 
  select(-YS_delta_icedtea_24w, -YS_delta_alcohol_24w, -YS_delta_salty_24w,
         -YS_delta_fats_24w, -YS_delta_ldl_24w, -YS_delta_tg_24w)
```
...
```{r}
df46 = read_rds("cleaned_data/trial46.rds")
```
...
```{r}
df47 = read_rds("cleaned_data/trial47.rds")
```
...
df_comparison = update_df(47, c(rep("binary", n)))
```
...
```{r}
df48 = read_rds("cleaned_data/trial48.rds") %>% 
  select(-YP_ap_ws_5m, -YP_ew_ss_5m, -YP_EF3_RT_5m, -YP_EF3_perccorr_5m, -YS_fb_p2_percCorr_5m, -YS_ravens_sum_score_5m, -YS_RCN_MeanTime_R_5m, -YS_Time1_h_5m)
```
...
df_comparison = update_df(48, c(rep("continuous", n)))
```
...
```{r}
df49 = read_rds("cleaned_data/trial49.rds") %>% 
...
```{r}
df50 = read_rds("cleaned_data/trial50.rds")
```
...
df_comparison = update_df(50, rep("continuous", n))
```
...
# 4. Precision Gain Plot (Super Learner) - Using color
...
# 5. Precision Gain Plot (Super Learner, High Outliers Removed) - Using color
ggplot(df_new %>%
...
# 6. Point Estimate Difference Plot (Super Learner) - Using color
