...
for (i in 1:nrow(fig_configs)) {
  ...
  cat(sprintf("Creating figure %d/6: %s...\n", i, fig_name))
  ...
  ggsave(filename, combined_fig, 
         width = 30, height = 12, dpi = pub_dpi, bg = "white")
...
# N. Cova vs N Plot
```{r}
# Create data frame with sample size and number of covariates
df_cov_size = data.frame(
  sample_size = df_meta$`Sample Size`,
  n_covariates = df_meta$No_Covariates
) %>%
  filter(!is.na(sample_size), !is.na(n_covariates))

# Calculate correlation
correlation = cor(df_cov_size$sample_size, df_cov_size$n_covariates, 
                  use = "complete.obs")

# Create scatter plot with smoothed curve
p_cov_size = ggplot(df_cov_size, aes(x = sample_size, y = n_covariates)) +
  geom_point(alpha = 0.6, size = 3, color = "#2196F3") +
  geom_smooth(method = "loess", se = TRUE, color = "#FF5722", 
              fill = "#FF5722", alpha = 0.2, linewidth = 1.2) +
  labs(x = "Sample Size",
...
# Create scatter plot with smoothed curve (log10 scale)
p_cov_size_log = ggplot(df_cov_size, aes(x = log10(sample_size), y = n_covariates)) +
  geom_point(alpha = 0.6, size = 3, color = "#2196F3") +
  geom_smooth(method = "loess", se = TRUE, color = "#FF5722", 
              fill = "#FF5722", alpha = 0.2, linewidth = 1.2) +
  labs(x = "Log10(Sample Size)",
