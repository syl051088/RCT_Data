...
```{r}
df_bench = update_bench(1, df1_yp, outcome)
```
...
```{r}
df_bench = update_bench(1, df1_ys, outcome)
```
...
```{r}
df_bench = update_bench(2, df2_yp, outcome)
```
...
```{r}
df_bench = update_bench(2, df2_ys, outcome)
```
...
```{r}
df_bench = update_bench(3, df3_yp, "YP_composite_time")
```
...
```{r}
df_bench = update_bench(3, df3_ys, outcomes)
```
...
```{r}
# df_bench = update_bench(4, df4, outcome)
```
...
```{r}
df_bench = update_bench(5, df5_yp, outcome)
```
...
```{r}
df_bench = update_bench(5, df5_ys, outcome)
```
...
```{r}
df_bench = update_bench(6, df6_yp, outcome)
```
...
df_comparison = update(7, results, rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```


```{r}
# df_bench = update_bench(7, df7, outcome)
```
...
```{r}
df_bench = update_bench(8, df8_yp, "YP_RTPCR_negative_6d")
```
...
```{r}
df_bench = update_bench(8, df8_ys, outcome)
```
...
```{r}
df_bench = update_bench(9, df9_yp, "YP_manual_removal")
```
...
```{r}
df_bench = update_bench(9, df9_ys, outcome)
```
...
df_comparison = update(10, results, "binary", SL_methods = c("SL.rpart", "SL.ranger"))
```


```{r}
# df_bench = update_bench(10, df10_yp, "YP_help_needed_intub")
```
...
df_comparison = update(10, results, type, SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(10, df10_ys, outcome)
```
...
df_comparison = update(11 ,results, rep("continuous", 2), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(11, df11_yp, outcome)
```
...
df_comparison = update(11, results, rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(11, df11_ys, outcome)
```
...
df_comparison = update(12, results, c("binary", rep("continuous", 5)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(12, df12_yp, outcome, covariates)
```
...
df_comparison = update(12, results, rep("continuous", 2), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(12, df12_ys, outcome, covariates)
```
...
```{r}
df_bench = update_bench(13, df13_yp, outcome)
```
...
```{r}
df_bench = update_bench(13, df13_ys, outcome)
```
...
df_comparison = update(14, results, rep("continuous", 2), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(14, df14_yp, outcome)
```
...
df_comparison = update(14, results, c(rep("continuous", length(outcome)-5), rep("binary", 5)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(14, df14_ys, outcome)
```
...
df_comparison = update(15, results, rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(15, df15_yp, outcome)
```
...
```{r}
# df_bench = update_bench(15, df15_ys, outcome)
```
...
df_comparison = update(16, results, rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(16, df16_yp, outcome)
```
...
```{r}
# df_bench = update_bench(16, df16_ys, outcome)
```
...
```{r}
df_bench = update_bench(17, df17_yp, "YP_delta_LDL_24w")
```
...
```{r}
df_bench = update_bench(17, df17_ys, outcome)
```
...
```{r}
df_bench = update_bench(18, df18_yp, c("YP_time_to_disengaged_12m", "YP_prop_time_in_care_12m"))
```
...
df_comparison = update(18, results, rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(18, df18_ys, outcome)
```
...
```{r}
# df_bench = update_bench(19, df19_yp, "YP_FHVP_CVP_GRADIENT")
```
...
```{r}
# df_bench = update_bench(19, df19_ys, outcome)
```
...
df_comparison = update(20, analyze_rct(df20_ys, outcome), rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(20, df20_ys, outcome)
```
...
df_comparison = update(21, analyze_rct(df21_yp, outcome), "time to event", SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(21, df21_yp, outcome)
```
...
df_comparison = update(21, analyze_rct(df21_ys, outcome), c(rep("binary", 3), "continuous"), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(21, df21_ys, outcome)
```
...
df_comparison = update(22, analyze_rct(df22_yp, "YP_Birthwgt", covariates), "continuous", SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(22, df22_yp, "YP_Birthwgt", covariates)
```
...
```{r}
df_bench = update_bench(23, df23_yp, c("YP_time_28d", "YP_time_90d"))
```
...
```{r}
df_bench = update_bench(23, df23_ys, outcome)
```
...
df_comparison = update(24, results, "binary", SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(24, df24_yp, "YP_catheterized")
```
...
df_comparison = update(24, analyze_rct(df24_ys, outcome), rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(24, df24_ys, outcome)
```
...
df_comparison = update(25, analyze_rct(df25_yp, outcome), rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(25, df25_yp, outcome)
```
...
df_comparison = update(26, analyze_rct(df26_yp, outcome), c("continuous", "categorical", "categorical"), SL_methods = c("SL.rpart", "SL.ranger"))
```

```{r}
# df_bench = update_bench(26, df26_yp, outcome)
```
...
df_comparison = update(26, analyze_rct(df26_ys, outcome), rep("continuous", length(outcome)), SL_methods = c("SL.rpart", "SL.ranger"))
```
...
```{r}
df_comparison = update(28, analyze_rct(df28_yp, "YP_ContrSBP_6m"), "binary", SL_methods = c("SL.rpart", "SL.ranger"))
```
...
df_comparison = update(28, analyze_rct(df28_ys, outcome), 
                       c(rep("continuous", 7), "binary", "continuous", "binary", 
                         "continuous", "continuous"), SL_methods = c("SL.rpart", "SL.ranger"))
```
...
df_comparison = update(29, analyze_rct(df29_yp, outcome),
                       c("continuous", "binary", rep("continuous", 4)), SL_methods = c("SL.rpart", "SL.ranger"))
```
...
df_comparison = update(29, analyze_rct(df29_ys, outcome),
                       rep("continuous", 2), SL_methods = c("SL.rpart", "SL.ranger"))
```
...
```{r}
df_comparison = update(30, analyze_rct(df30_yp, "YP_Adherence"), "continuous", SL_methods = c("SL.rpart", "SL.ranger"))
```
...
df_comparison = update(30, analyze_rct(df30_ys, outcome),
                       rep("continuous", 7), SL_methods = c("SL.rpart", "SL.ranger"))
```
