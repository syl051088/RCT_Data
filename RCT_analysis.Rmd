...
df_long_full = df_new %>%       # df_new in your workspace
  ...
  mutate(method = dplyr::case_match(
    ...
    "precision_gain_SL_nnet"         ~ "SL_nnet",
    ...
    "precision_gain_SL_nnet_VS"      ~ "SL_nnet_VS"
  ))

###############################################################################
##  1.  Caption builder for ONE outcome type ----------------------------------
###############################################################################
build_caption = function(outcome_type) {
  ...
  if (outcome_type == "continuous")
    nas = nas %>% filter(!method %in% c("Binom", "Binom_VS"))
...
###############################################################################
df_binary_or = df_long_outlier_removed %>% filter(outcome_group == "binary")
cap_binary   = build_caption("binary")

p_binary = make_plot(
  df_binary_or,
  "Precision Gain vs Unadjusted – Binary",
  cap_binary
)
ggsave("cleaned_data/Plot/precision_gain_binary.png", p_binary, width = 12, height = 6, dpi = 1200)

###############################################################################
##  4.  Continuous plot -------------------------------------------------------
###############################################################################
df_continuous_or = df_long_outlier_removed %>% filter(outcome_group == "continuous")
cap_cont         = build_caption("continuous")

p_cont = make_plot(
  df_continuous_or,
  "Precision Gain vs Unadjusted – Continuous",
  cap_cont
)
ggsave("cleaned_data/Plot/precision_gain_continuous.png", p_cont, width = 12, height = 6, dpi = 1200)
