...
###############################################################################
# Define color palette that matches the smooth plots (AIPW removed)
method_colors = c(
  "ANCOVA" = "#F8766D",
  "ANHECOVA" = "#B79F00",
  "G-Logistic" = "#00BA38",
...
make_plot = function(data, outcome_type) {
  ...
  # Set factor levels based on outcome type (AIPW removed)
  ...
  p = ggplot(data, aes(method, prop_var_reduction, fill = method)) +
    ...
    theme(
      ...
      axis.title.y  = element_text(size = 18),  # Removed face = "bold"
...
prepare_long_data_with_size = function(data) {
  data %>%
    ...
    mutate(method = dplyr::case_match(
      ...
      # Top-3 methods (no line break, AIPW removed)
      ...
      # Baseline+ methods (no line break, AIPW removed)
...
make_plot_smooth_log = function(data, outcome_type) {
  if (outcome_type == "binary") {
    method_levels = c("ANCOVA", "ANHECOVA", "G-Logistic", "DML", "TMLE", "IPW",
                     "ANCOVA Top-3", "ANHECOVA Top-3", "G-Logistic Top-3", "DML Top-3", "TMLE Top-3", "IPW Top-3",
                     "ANCOVA Baseline+", "ANHECOVA Baseline+", "G-Logistic Baseline+", "DML Baseline+", "TMLE Baseline+", "IPW Baseline+")
    data = data %>% mutate(method = factor(method, levels = method_levels))
  } else {
    # For continuous, exclude G-Logistic
    method_levels = c("ANCOVA", "ANHECOVA", "DML", "TMLE", "IPW",
                     "ANCOVA Top-3", "ANHECOVA Top-3", "DML Top-3", "TMLE Top-3", "IPW Top-3",
                     "ANCOVA Baseline+", "ANHECOVA Baseline+", "DML Baseline+", "TMLE Baseline+", "IPW Baseline+")
    data = data %>% mutate(method = factor(method, levels = method_levels))
  }
...
combined_width = 22
combined_height = 18  # Increased height for 3 rows
