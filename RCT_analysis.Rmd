...
update_df_tmle = function(trial_no,
  ...
  for (i in seq_len(nrow(results))) {
    ...
    if (suffix == "") {
      ...
      tmle_se = row_i$Est_SL
      tmle_est = row_i$SE_SL
...
prepare_long_data = function(data) {
  data %>%
    ...
    mutate(method = dplyr::case_match(
      ...
      "precision_gain_AIPW_Top-3"                   ~ "AIPW\nTop-3",
      ...
      "precision_gain_AIPW_Baseline+"               ~ "AIPW\nBaseline+",
...
make_plot = function(data, outcome_type) {
  ...
  if (outcome_type == "binary") {
    # Binary: ANCOVA, ANHECOVA, G-Logistic, DML, TMLE, AIPW, IPW
    method_levels = c("ANCOVA", "ANHECOVA", "G-Logistic", "DML", "TMLE", "AIPW", "IPW",
                     "ANCOVA\nTop-3", "ANHECOVA\nTop-3", "G-Logistic\nTop-3", "DML\nTop-3", "TMLE\nTop-3", "AIPW\nTop-3", "IPW\nTop-3",
                     "ANCOVA\nBaseline+", "ANHECOVA\nBaseline+", "G-Logistic\nBaseline+", "DML\nBaseline+", "TMLE\nBaseline+", "AIPW\nBaseline+", "IPW\nBaseline+")
  } else {
    # Continuous: ANCOVA, ANHECOVA, DML, TMLE, AIPW, IPW (no G-Logistic)
    method_levels = c("ANCOVA", "ANHECOVA", "DML", "TMLE", "AIPW", "IPW",
                     "ANCOVA\nTop-3", "ANHECOVA\nTop-3", "DML\nTop-3", "TMLE\nTop-3", "AIPW\nTop-3", "IPW\nTop-3",
                     "ANCOVA\nBaseline+", "ANHECOVA\nBaseline+", "DML\nBaseline+", "TMLE\nBaseline+", "AIPW\nBaseline+", "IPW\nBaseline+")
  }
  ...
  # Add method labels as geom_text (bold)
  p = p + 
...
method_colors = c(
  ...
  "ANHECOVA Baseline+" = "#B79F00",
  "G-Logistic Baseline+" = "#00BA38",
  "DML Baseline+" = "#00BFC4",
  "TMLE Baseline+" = "#619CFF",
  "AIPW Baseline+" = "#F564E3",
  "IPW Baseline+" = "#FF6F00"
)
...
prepare_long_data_with_size = function(data) {
  data %>%
    ...
    mutate(method = dplyr::case_match(
      ...
      "precision_gain_AIPW_Top-3"                   ~ "AIPW Top-3",
      "precision_gain_IPW_Top-3"                    ~ "IPW Top-3",
      
      # Baseline+ methods (no line break)
      ...
      "precision_gain_AIPW_Baseline+"               ~ "AIPW Baseline+",
      "precision_gain_IPW_Baseline+"                ~ "IPW Baseline+"
    )) %>%
    filter(!is.na(method))
}
...
make_plot_smooth_log = function(data, outcome_type) {
  if (outcome_type == "binary") {
    method_levels = c("ANCOVA", "ANHECOVA", "G-Logistic", "DML", "TMLE", "AIPW", "IPW",
                     "ANCOVA Top-3", "ANHECOVA Top-3", "G-Logistic Top-3", "DML Top-3", "TMLE Top-3", "AIPW Top-3", "IPW Top-3",
                     "ANCOVA Baseline+", "ANHECOVA Baseline+", "G-Logistic Baseline+", "DML Baseline+", "TMLE Baseline+", "AIPW Baseline+", "IPW Baseline+")
    data = data %>% mutate(method = factor(method, levels = method_levels))
  } else {
    # For continuous, exclude G-Logistic
    method_levels = c("ANCOVA", "ANHECOVA", "DML", "TMLE", "AIPW", "IPW",
                     "ANCOVA Top-3", "ANHECOVA Top-3", "DML Top-3", "TMLE Top-3", "AIPW Top-3", "IPW Top-3",
                     "ANCOVA Baseline+", "ANHECOVA Baseline+", "DML Baseline+", "TMLE Baseline+", "AIPW Baseline+", "IPW Baseline+")
    data = data %>% mutate(method = factor(method, levels = method_levels))
  }
...
dot = "
digraph RCT_DataProcessing {
  graph [rankdir=LR, fontsize=18, splines=ortho, nodesep=0.6, ranksep=0.5]
  node  [shape=rect, fontname=Helvetica, fontsize=16, style=filled, fillcolor=white]
  edge  [arrowhead=normal, penwidth=1.4]

  # Stage 1: Raw and import
  A [label='Raw dataset(s)\n(downloaded from Dryad / Dataverse)']

  # Stage 2: Cleaning
  B [label='Clean missing values\n(. , NULL , blank → NA)']
  C [label='Define Treatment variable']
  D [label='Rename outcomes\n(YP_/YS_/X_ convention)']

  # Stage 3: Structuring
  E [label='Reshape to wide format\n(1 row per subject)']
  F [label='Cluster-level aggregation\n(mean within cluster)']
  G [label='Save cleaned & cluster-level datasets']
