...
analyze_rct = function(df,
  ...
  for (i in seq_along(outcome_cols)) {
    ...
    # Variable selection: select top 3 covariates by outcome-specific correlation
    if (selection && length(valid_covariates) > 0) {
      if (length(valid_covariates) > 3) {
        # Filter to numeric covariates for correlation computation
        numeric_covariates = character(0)
        for (cov in valid_covariates) {
          if (is.numeric(df_complete[[cov]])) {
            numeric_covariates = c(numeric_covariates, cov)
          }
        }
        
        if (length(numeric_covariates) >= 3) {
          # Get correlations between this outcome and numeric covariates
          cov_values = numeric(length(numeric_covariates))
          names(cov_values) = numeric_covariates
          
          for (cov in numeric_covariates) {
            cov_values[cov] = cor(df_complete[[outcome]], df_complete[[cov]], use = "complete.obs")
          }
          
          # Select top 3 numeric covariates by absolute correlation magnitude
          top_3_idx = order(abs(cov_values), decreasing = TRUE)[1:3]
          selected_numeric_covariates = numeric_covariates[top_3_idx]
          
          # Use selected numeric covariates plus any factor covariates (up to 3 total)
          factor_covariates = setdiff(valid_covariates, numeric_covariates)
          all_selected = c(selected_numeric_covariates, factor_covariates)
          valid_covariates = all_selected[1:min(3, length(all_selected))]
          
          cat("Variable selection for", outcome, "- Selected covariates:", 
              paste(valid_covariates, collapse = ", "), "\n")
        } else {
          # If <3 numeric covariates, use all valid covariates
          cat("Variable selection for", outcome, "- Using all", length(valid_covariates), 
              "covariates (<3 numeric available):", paste(valid_covariates, collapse = ", "), "\n")
        }
      } else {
        # If ≤3 valid covariates, use all of them
        cat("Variable selection for", outcome, "- Using all", length(valid_covariates), 
            "covariates (≤3):", paste(valid_covariates, collapse = ", "), "\n")
      }
    }
...
update_df = function(trial_no,
  ...
  unique_outcomes = unique(res1$Outcome)
  ...
  new_rows = res1_renamed %>%
    ...
    mutate(
      ...
      `How much precision gain can ANCOVA provide?` =
        (ANCOVA_robust_se / Unadjust_se) ^ 2,
      `The difference between unadjusted and ANCOVA point estimates` =
        (ANCOVA_est - Unadjust_est) / sqrt((ANCOVA_robust_se ^ 2 + Unadjust_se ^ 2) / 2),
      ...
      `How much precision gain can Super Learner provide?` =
        (SuperLearner_se / Unadjust_se) ^ 2,
      `The difference between unadjusted and Super Learner point estimates` =
        (SuperLearner_est - Unadjust_est) / sqrt((SuperLearner_se ^ 2 + Unadjust_se ^ 2) / 2),
      `How much precision gain can ANHECOVA provide?` =
        (ANHECOVA_robust_se / Unadjust_se) ^ 2,
      `ANCOVA vs ANHECOVA variance ratio` =
        (ANCOVA_robust_se / ANHECOVA_robust_se) ^ 2
    ) %>%
    select(
      Trial_No,
      Sample_Size,
...
size_colors <- c("Small (<50)" = "#d73027", "Medium (50-99)" = "#fc8d59", 
                "Large (100-499)" = "#91bfdb", "Very Large (≥500)" = "#4575b4")
