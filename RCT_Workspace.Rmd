...
cat("\n<------------------Variable Selection------------->\n\n")
res4 = analyze_rct_tmle(df1, selection = TRUE, variable_selection_type = 1)
```
...
# Latest TMLE
```{r}
## analyze_rct_tmle() - Enhanced version with variable selection methods
...
```{r}
# ============================================================================
# TMLE Analysis with Variable Selection Type 2 for All 50 Trials
# ============================================================================

# Manual covariates for each trial (extracted from your RMD file)
manual_covariates_list = list(
  ...
  df11 = c("X_sex_0w"),
  df12 = c("X_AuthorGender_0w"),
  df13 = c("X_AGE_0min"),
  df14 = c("X_center_0w", "X_sex_0w", "X_age_0w"),
  df15 = c(),  # Empty - no demo variables
  ...
  df21 = c(),  # Empty - no demo variables
  df22 = c("X_age_0w"),
  df23 = c(),  # Empty - no demo variables
  df24 = c("X_Age_0d", "X_Weight_0d"),
  df25 = c("X_Sex_0w", "X_BMI_0w"),
  df26 = c("X_Agecat_0m", "X_weight_0m"),
  df27 = c(),  # Empty - no demo variables
  ...
  df35 = c("X_BMI_0w"),
  df36 = c(),  # Empty - no demo variables
  ...
  df48 = c("X_age_group", "X_sex"),
  df49 = c("X_gestAge_0w", "X_birthWt_0d"),
  df50 = c("X_sex_0w", "X_center_0w", "X_age_0w", "X_weight_0w")
)

# Initialize empty dataframe to store all results
final_tmle_results = data.frame()

# Main execution loop
for (i in 1:50) {
  df_name = paste0("df", i)
  
  cat("\n==================== TMLE Analysis for", df_name, "====================\n")
  cat("<------------------Variable Selection Type 2 (Manual + Baseline)------------->\n\n")
  
  # Load the dataframe
  df = get(df_name)
  
  # Get manual covariates for this trial
  manual_covs = manual_covariates_list[[df_name]]
  
  if (is.null(manual_covs)) {
    manual_covs = character(0)  # Empty vector if NULL
  }
  
  cat("Manual covariates for", df_name, ":", 
      ifelse(length(manual_covs) > 0, paste(manual_covs, collapse = ", "), "None"), "\n\n")
  
  # Run TMLE analysis with VS2
  tryCatch({
    res_tmle = analyze_rct_tmle(df, 
                               selection = TRUE, 
    ...
    res_tmle$Trial = i
    res_tmle$DataFrame = df_name
    
    # Add rows to final results
    final_tmle_results = rbind(final_tmle_results, res_tmle)
    
    # Print results summary
    cat("TMLE Results Summary:\n")
    cat("- Number of outcomes analyzed:", nrow(res_tmle), "\n")
    cat("- Outcomes with valid estimates:", sum(!is.na(res_tmle$Est_SL)), "\n")
    cat("- Outcomes with missing estimates:", sum(is.na(res_tmle$Est_SL)), "\n\n")
    
  }, error = function(e) {
    cat("ERROR in", df_name, ":", e$message, "\n")
  })
  
  cat("Completed", df_name, "\n")
}

# Final summary
cat("\n==================== Final Results ====================\n")
if (nrow(final_tmle_results) > 0) {
  cat("Total outcome analyses:", nrow(final_tmle_results), "\n")
  cat("Valid TMLE estimates:", sum(!is.na(final_tmle_results$Est_SL)), "\n")
  cat("Missing TMLE estimates:", sum(is.na(final_tmle_results$Est_SL)), "\n\n")
  
  # Show summary by trial
  library(dplyr)
  summary_by_trial = final_tmle_results %>%
    group_by(Trial, DataFrame) %>%
    summarise(
      n_outcomes = n(),
      valid_estimates = sum(!is.na(Est_SL)),
      .groups = "drop"
    )
  ...
  # Save results
  write.csv(final_tmle_results, "cleaned_data/tmle_vs2_all_trials_results.csv", row.names = FALSE)
  cat("\nAll results saved to: tmle_vs2_all_trials_results.csv\n")
...
```{r}
# AIPW Wrapper Demo
# This demonstrates what results aipw_wrapper returns
...
# Create a simple simulated dataset (n=200)
n <- 200
...
# Generate treatment based on covariates (confounding)
p_treat <- plogis(-0.5 + 0.5*W1 + 0.3*W2)
A <- rbinom(n, 1, p_treat)

# Generate outcome based on treatment and covariates
# True treatment effect: ATE = 0.2 on probability scale
p_outcome <- plogis(-1 + 0.5*A + 0.4*W1 + 0.3*W2)
Y <- rbinom(n, 1, p_outcome)

# Combine covariates into a data frame
W <- data.frame(W1 = W1, W2 = W2)

# Run AIPW wrapper with simple SuperLearner libraries
# Using basic algorithms for quick demonstration
result <- aipw_wrapper(
  Y = Y,
  A = A,
  W = W,
  Q.SL.library = c("SL.glm", "SL.mean"),  # Outcome model libraries
  g.SL.library = c("SL.glm", "SL.mean"),  # Propensity score libraries
  k_split = 5,                             # 5-fold cross-fitting
  g.bound = 0.025,                         # Propensity score truncation
  verbose = TRUE,
...
print("=" * 60)
print("AIPW RESULTS SUMMARY")
print("=" * 60)

# The result object contains:
# 1. result$result - Main estimates table
print("\nMain Estimates Table:")
print(result$result)

# 2. result$estimates - List with detailed estimates
print("\nDetailed Estimates:")
print(names(result$estimates))
print("\nRisk Difference:")
print(result$estimates$RD)
print("\nRisk Ratio:")
print(result$estimates$RR)
print("\nOdds Ratio:")
print(result$estimates$OR)

# 3. You can also access:
# - result$obs_est: Individual-level estimates (propensity scores, predictions, etc.)
# - result$n: Sample size
# - result$libs: SuperLearner fitted objects (if save.sl.fit=TRUE)

print("\nSample size:")
print(result$n)

print("\nAvailable components in result object:")
print(names(result))
```

```{r}
# AIPW Wrapper Demo - CONTINUOUS OUTCOME
# This demonstrates what results aipw_wrapper returns for continuous Y
...
# Generate CONTINUOUS outcome based on treatment and covariates
# True treatment effect (ATE) = 5 units
# Y = baseline (20) + treatment effect (5) + covariate effects + noise
Y <- 20 + 5*A + 3*W1 + 2*W2 + rnorm(n, mean = 0, sd = 2)
...
result$estimates$RD[[1]]

result$estimates$RD[[2]]
```
...
# left_weight > right_weight makes the left panel wider
...
create_4panel_figure = function(outcome_type, vs_type) {
  ...
  label_all = ggplot() + annotate("text", x=0, y=0, label="All outcome analysis",
                                  size=8, fontface="bold", angle=90) + base_label_theme
  label_primary = ggplot() + annotate("text", x=0, y=0, label="Primary outcome analysis",
                                      size=8, fontface="bold", angle=90) + base_label_theme

  # build rows; tweak left/right weights here if you like
  top_row = row_with_label(label_all,     p_violin_all,     p_smooth_all,
                           label_width = 0.11, left_weight = 1.25, right_weight = 1.00)
  bot_row = row_with_label(label_primary, p_violin_primary, p_smooth_primary,
                           label_width = 0.11, left_weight = 1.25, right_weight = 1.00)

  combined = top_row / bot_row + plot_layout(heights = c(1, 1))
  combined
}

# --- saving unchanged, maybe slightly narrower width since label col is tiny
combined_width = 20
combined_height = 12
...
## run_tmle
```{r}
run_tmle_analysis = function(df_indices = 1:50, selection = T, variable_selection_type = 2) {
...
# sss
```{r}
get_outcome_types <- function(data, outcomes) {
  sapply(outcomes, function(var) {
    x <- data[[var]]
    if (is.numeric(x)) {
      "continuous"
...
# ---- Example ----
# outcomes is your vector of variable names
# e.g.
# outcomes <- c("YS_SmokingStatus_6m", "YP_BMI_6m", "YS_Region")

outcome_types <- get_outcome_types(df12_final, outcomes)
outcome_types
```
...
check_multicollinearity_cont <- function(data, prefix = "X_", vif_threshold = 5, corr_threshold = 0.9) {
  # 1) Grab only numeric variables starting with prefix
  xdat <- data %>%
    select(starts_with(prefix)) %>%
    select(where(is.numeric))
  
  if (ncol(xdat) == 0)
    stop("No numeric columns found starting with '", prefix, "'.")

  # 2) Drop all-NA and zero-variance columns
  nzv <- vapply(xdat, function(z) !all(is.na(z)) && (sd(z, na.rm = TRUE) > 0), logical(1))
  xdat <- xdat[, nzv, drop = FALSE]
  if (ncol(xdat) == 0)
    stop("All selected numeric columns are all-NA or zero-variance after filtering.")

  # 3) Correlation matrix (for inspection)
  cor_mat <- cor(xdat, use = "pairwise.complete.obs")
  
  # 4) Dummy regression to compute VIF (depends only on X)
  y_dummy <- rnorm(nrow(xdat))
  fit <- lm(y_dummy ~ ., data = xdat)
  
  vif_vals <- car::vif(fit)
  vif_tbl  <- sort(vif_vals, decreasing = TRUE)
  high_vif <- vif_tbl[vif_tbl > vif_threshold]

  # 5) Pairwise correlations exceeding threshold
  idx <- which(abs(cor_mat) > corr_threshold & abs(cor_mat) < 1, arr.ind = TRUE)
  corr_pairs <- if (nrow(idx) > 0) {
    unique(data.frame(
...
# ---- Usage ----
res <- check_multicollinearity_cont(df12_final, prefix = "X_", vif_threshold = 5, corr_threshold = 0.9)

# Inspect results
res$vif              # All VIFs sorted high → low
res$high_vif         # Continuous vars with VIF > 5
res$high_corr_pairs  # High correlations (|r| > 0.9)
...
analyze_rct_ipw = function(df,
  ...
  cat("DEBUG: Found", length(covariates), "covariates\n")
  ...
  cat("DEBUG: Treatment arms:", paste(arms, collapse = ", "), "\n")
  cat("DEBUG: Control level:", control_level, "\n")
  cat("DEBUG: Treatment levels:", paste(treatment_levels, collapse = ", "), "\n")
  ...
  for (i in seq_along(outcome_cols)) {
    ...
    # Check missing proportion for outcome
    missing_prop = mean(is.na(df[[outcome]]))
    cat("Missing proportion:", round(missing_prop, 3), "\n")
    ...
    for (cov in covariates) {
      if (is.factor(df_complete[[cov]]) &&
          length(levels(droplevels(df_complete[[cov]]))) <= 1) {
        cat("REMOVED covariate:", cov, "- Only one level after filtering (outcome:",
            outcome, ")\n")
      } else {
        valid_covariates = c(valid_covariates, cov)
      }
    ...
    cat("Valid covariates:", length(valid_covariates), "\n")
    ...
    cat("Outcome class:", class(df_complete_numeric[[outcome]]), "\n")
    cat("Outcome range:", range(df_complete_numeric[[outcome]], na.rm = TRUE), "\n")
    
    # Variable selection
    if (selection && length(valid_covariates) > 0) {
      if (variable_selection_type == 1) {
        selected_covariates = variable_selection_correlation(
          outcome, valid_covariates, df_complete_numeric, n_select
        )
      } else if (variable_selection_type == 2) {
        ...
        if (length(selected_covariates) == 0) {
          cat("No valid covariates found for outcome:", outcome, 
              "- Setting all estimates to NA\n")
      ...
      valid_covariates = selected_covariates
      cat("Selected covariates for", outcome, ":", 
    ...
    for (j in seq_along(treatment_levels)) {
      ...
      cat("\n--- Comparing", treat_level, "vs", control_level, "---\n")
      
      # Subset data
      df_pairwise = df_complete_numeric %>%
        filter(!!sym(treatment_col) %in% c(control_level, treat_level))
      
      cat("Pairwise sample size:", nrow(df_pairwise), "\n")
      
      # Create binary treatment
      df_pairwise_binary = df_pairwise
      df_pairwise_binary$treatment_binary = as.numeric(
        df_pairwise_binary[[treatment_col]] == treat_level
      )
      
      cat("Binary treatment distribution:", 
          table(df_pairwise_binary$treatment_binary), "\n")
      
      # FIX: Use eval() and parse() to handle formula properly
      cat("Estimating IPW weights...\n")
      
      if (length(valid_covariates) > 0) {
        # Build the formula string
        formula_str = paste("~", paste(valid_covariates, collapse = " + "))
        cat("Denominator formula string:", formula_str, "\n")
        
        # Use do.call to pass the formula directly
        weights_obj = do.call(ipwpoint, list(
          exposure = quote(treatment_binary),
          family = "binomial",
          link = "logit",
          numerator = ~ 1,
          denominator = as.formula(formula_str),
          data = quote(df_pairwise_binary),
          trunc = trunc
        ))
      } else {
        weights_obj = ipwpoint(
          exposure = treatment_binary,
          family = "binomial",
          link = "logit",
          numerator = ~ 1,
          denominator = ~ 1,
          data = df_pairwise_binary,
          trunc = trunc
        )
      }
      
      cat("Weight summary:\n")
      print(summary(weights_obj$ipw.weights))
      
      # Add weights to data
      df_pairwise_binary$ipw = weights_obj$ipw.weights
      
      cat("Fitting weighted outcome model...\n")
      
      # Fit weighted model
      design = svydesign(~ 1, weights = ~ ipw, data = df_pairwise_binary)
      model = svyglm(
        as.formula(paste(outcome, "~ treatment_binary")),
        design = design
      )
      ...
      # Extract ATE and SE
      est = coef(model)["treatment_binary"]
      se = sqrt(diag(vcov(model)))["treatment_binary"]
      
      cat("ATE Estimate:", est, "\n")
      cat("SE:", se, "\n")
...
```{r}
res1 = analyze_rct_ipw(df1, F, 1)
res2 = analyze_rct_ipw(df1, T, 1)
res3 = analyze_rct_ipw(df1, T, 2, demo)
```
...
confounder1 <- rnorm(n, mean = 50, sd = 10)
confounder2 <- rbinom(n, 1, 0.5)
confounder3 <- rnorm(n, mean = 100, sd = 15)

# Treatment assignment depends on confounders
treatment_logit <- -2 + 0.05*confounder1 + 0.3*confounder2 + 0.02*confounder3
treatment_prob <- exp(treatment_logit) / (1 + exp(treatment_logit))
treatment_var <- rbinom(n, 1, treatment_prob)

# Outcome depends on treatment AND confounders
# True ATE = 10
outcome <- 10*treatment_var + 
           0.5*confounder1 + 
           5*confounder2 + 
           0.3*confounder3 + 
           rnorm(n, 0, 15)
...
weights <- ipwpoint(
  exposure = treatment_var,
  family = "binomial",
  link = "logit",
  numerator = ~ 1,
  denominator = ~ confounder1 + confounder2 + confounder3,
  data = your_data
)

# Add weights to dataset
your_data$ipw <- weights$ipw.weights

# Fit weighted outcome model with robust SE
design <- svydesign(~ 1, weights = ~ ipw, data = your_data)
ate_model <- svyglm(outcome ~ treatment_var, design = design)

# Extract ATE and SE
ate_est <- coef(ate_model)["treatment_var"]
ate_se <- sqrt(diag(vcov(ate_model)))["treatment_var"]

# Report results
cat("══════════════════════════════════════════════════════════\n")
cat("IPW Results (True ATE = 10)\n")
cat("══════════════════════════════════════════════════════════\n")
cat("ATE Estimate:", round(ate_est, 3), "\n")
cat("Standard Error:", round(ate_se, 3), "\n")
cat("95% CI: [", round(ate_est - 1.96*ate_se, 3), ",", 
    round(ate_est + 1.96*ate_se, 3), "]\n")
cat("══════════════════════════════════════════════════════════\n")
```
```{r}
df_comparison %>% 
  filter(`Outcome Type` %in% c("continuous", "binary", "continuous proportion", "composite binary")) %>% 
  nrow()
```
