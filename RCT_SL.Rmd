...
analyze_rct_multiarm = function(data,
  ...
  results = map_dfr(outcome_cols, function(Y) {
    map_dfr(setdiff(arms, reference_arm), function(arm) {
      # define the two-arm set and conditional pis
      ...
      cv_comp = map_dfr(1:K, function(k) {
        ...
        for (a in pair_arms) {
          ...
          # fit SuperLearner on arm=a training set
      ...
      # compute contrast D = f_arm - f_ref
      D     = cv_comp[[paste0("f_", make.names(arm))]] -
        cv_comp[[paste0("f_", make.names(reference_arm))]]
